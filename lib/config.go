...
type ShipperConfig struct {
	...
	BatchTimeout    float64 `long:"batch-timeout-seconds" description:"Maximum time in seconds to wait for messages from the internal buffer to be ready before making a web socket call with current metrics." default:"0.1"`
...
func ParseShipperConfig() (*ShipperConfig, error) {
	...
	file, err := os.Open(commandlineopts.ConfigFilePath)
	if err == nil {
