...
func TestConnectFail(t *testing.T) {
	pub, err := NewWebsocketPublisher("ws://127.0.0.1:12345/metrics", 1, 1, 1, 1, 1, 1, "admin", "zenoss")
	if err != nil {
...
func TestConnect(t *testing.T) {
	once.Do(startServer)
	defer clearBuffer()
	pub, err := NewWebsocketPublisher("ws://"+serverAddr+"/metrics", 1, 1, 1, 1, 1, 1, "admin", "zenoss")
	if err != nil {
		t.Fatalf("Could not create websocket publisher: %s", err)
	}
	if err := pub.Start(); err != nil {
...
func TestHitBatchSize(t *testing.T) {
	once.Do(startServer)
	defer clearBuffer()
	pub, err := NewWebsocketPublisher("ws://"+serverAddr+"/metrics", 1, 6, 3, 1, 1, 1, "admin", "zenoss")
	if err != nil {
		t.Fatalf("Could not create websocket publisher: %s", err)
	}
	go pub.Start()
